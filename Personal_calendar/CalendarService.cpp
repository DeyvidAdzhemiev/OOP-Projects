...
void CalendarService::unbook(int startDay, int startmonth, int startYear, int startMinute, int startHour) 
{
	for (unsigned i = 0; i < calendar.getMeeting().size(); i++) 
	{
		Meeting crrMeeting = calendar.getMeeting()[i];
		...
		if (isForThisDay) 
		{
			this->calendar.getMeeting().erase(this->calendar.getMeeting().begin() + i - 1);
			return;
		}
	}
